#!/usr/bin/env python

import random
import collections
import itertools
from collections import Counter
import argparse

from music_tools import Music, pitches_to_chord_type
from utils import weighted_choice, pairwise



def get_base_interval(interval):
    interval = interval % 12
    if interval > 6:
        interval = 12 - interval
    return interval


# def print_chord(chord):
#     result = ''
#     for pc in range(12):
#         if pc in chord:
#             result += '|'
#         else:
#             result += ' '
#     result = result * 10
#     return result


def print_chord(chord):
    result = ''
    for pc in range(12):
        if pc in chord:
            result += '{:<2} '.format(pc)
        else:
            result += '.  '
    result = result * 2
    return result


def pc_spell(root, chord_type):
    return [(p + root) % 12 for p in chord_type]


chord_types = [
    (0, 4, 7),
    (0, 3, 7),
    (0, 4, 7, 10),
    (0, 3, 7, 10),
    (0, 4, 7, 11),
    (0, 5, 7),
]
pitch_classes = range(12)


def make_all_transitions():
    """
    Returns this:

    transitions = {
        starting chord type: {
            n common tones: [
                (root motion, destination chord type),
                (root motion, destination chord type),
                ...
            ],
            n+1 common tones: [
                (root motion, destination chord type),
                (root motion, destination chord type),
                ...
            ],
            n+2 common tones: [
                (root motion, destination chord type),
                (root motion, destination chord type),
                ...
            ],
            ...
        },
        another starting chord type: {
            ...
        },
        ...
    }

    """

    transitions = {}
    for starting_chord_type in chord_types:
        transitions[starting_chord_type] = collections.defaultdict(list)

        for destination_chord_type in chord_types:
            for root_motion in pitch_classes:
                destination_pitch_classes = [(p + root_motion) % 12 for p in destination_chord_type]

                if starting_chord_type == tuple(sorted(destination_pitch_classes)):
                    continue

                n_common_tones = len(set(destination_pitch_classes).intersection(set(starting_chord_type)))

                transitions[starting_chord_type][n_common_tones].append((root_motion, destination_chord_type))
    return transitions

transitions = make_all_transitions()


def generate_chords(n=100):
    root = random.choice(pitch_classes)
    chord_type = random.choice(chord_types)

    chords = []

    for _ in range(n):
        n_common_tones_options = transitions[chord_type].keys()
        n_common_tones_options.remove(0)
        n_common_tones_options.remove(1)
        n_common_tones = random.choice(n_common_tones_options)
        options = transitions[chord_type][n_common_tones]

        root_motion, chord_type = random.choice(options)

        root = (root + root_motion) % 12

        chord = pc_spell(root, chord_type)
        chord.sort()
        chords.append(chord)

    return chords


class Movement1(object):
    def __init__(self):
        m = self.music = Music(
            part_names=(
                'violin',
                'flute',
                'oboe',
                'clarinet',
                'trumpet',
                'alto_saxophone',
                'vibraphone',
                'bass'
            ),
            starting_tempo_bpm=105
        )
        beat_duration = 60.0 / self.music.starting_tempo_bpm

        chords = generate_chords()

        for chord in chords:
            print print_chord(chord)

        # initial_duration = 4.0
        # for instrument in self.music.instruments:
        #     pitch_options = [p for p in instrument.safe_register if p % 12 in chords[0]]
        #     pitch = random.choice()
        #     instrument.add_note(pitch=pitch, duration=initial_duration)
        #     instrument.playing = True

        # pc_instrument_map = self.get_pc_instrument_map()

        # for a, b in pairwise(chords):
        #     duration = random.choice([1, 1, 1, 2])

        #     set_a = set(a)
        #     set_b = set(b)

        #     dropped_notes = list(set_a - set_b)
        #     new_notes = list(set_b - set_a)
        #     common_notes = list(set_a.intersection(set_b))

        #     for dropped_note in dropped_notes:
        #         for new_note in new_notes:
        #             interval = get_base_interval(dropped_note - new_note)

        #             if interval == 1:
        #                 instruments = pc_instrument_map[dropped_note]
        #                 instrument = random.choice(instruments)



        #     pc_instrument_map = self.get_pc_instrument_map()







        #     needs_decision = self.music.instruments[:]

        #     eligible = []
        #     for instrument in self.music.instruments:
        #         if instrument.playing:
        #             last_note = instrument[-1]
        #             last_pc = last_note.pitch % 12
        #             if last_pc in common_notes:
        #                 last_note.duration += duration
        #                 instrument.playing = True
        #                 needs_decision.remove(instrument)
        #             else:
        #                 eligible.append(instrument)
        #         else:
        #             eligible.append(instrument)

        #     while len(new_notes):
        #         instrument = random.choice(eligible)
        #         instrument.playing = True
        #         eligible.remove(instrument)
        #         pc = random.choice(new_notes)
        #         new_notes.remove(pc)
        #         pitch_options = [p for p in instrument.safe_register if p % 12 == pc]
        #         pitch = random.choice(pitch_options)
        #         instrument.add_note(pitch=pitch, duration=duration)
        #         needs_decision.remove(instrument)

        #     for instrument in needs_decision:
        #         instrument.playing = False
        #         instrument.add_note(pitch='rest', duration=duration)

    def get_pc_to_instrument_map(self):
        pc_to_instrument_map = collections.defaultdict(list)
        for instrument in self.music.instruments:
            pc_to_instrument_map[instrument[-1].pitch].append(instrument)
        return pc_to_instrument_map

    def notate(self):
        self.music.notate()


def command_line_interface():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '-d',
        '--dont-notate',
        help='dont generate notation',
        action="store_true")
    return parser.parse_args()


if __name__ == '__main__':
    args = command_line_interface()

    m1 = Movement1()
    if not args.dont_notate:
        m1.notate()
